...
def generate_missing_json():
    ...
    for im in os.listdir('images_train'):
        fn = im.split('.')[0]+'.png'+'.json'
        path = os.path.join('annotated_train', fn)
...
def generate_missing_json_val():

    # creates a background json for the entire image if missing
    # this assumes you will never annotate a background class

    for im in os.listdir('images_val'):
        fn = im.split('.')[0]+'.png'+'.json'
        path = os.path.join('annotated_val', fn)
